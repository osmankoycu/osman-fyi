...
export function Navbar() {
    ...
    return (
        <nav
            className={clsx(
                'w-full sticky top-0 z-50 flex items-center transition-[height] duration-300 backdrop-blur-md',
                isStuck ? 'h-[80px]' : 'h-[80px]',
        ...
        >
            <div className="container-text flex items-center justify-between w-full h-full">
                ...
                <div className="md:hidden flex items-center justify-between w-full">
                    <div className="flex items-center space-x-1 overflow-x-auto no-scrollbar -mx-5 px-5 mask-image-scroll w-full">
                        {navItems.map((item) => {
                            const isActive =
                                item.href === '/'
                                    ? pathname === '/' || pathname === '/product'
                            ...
                            return (
                                <Link
                                    ...
                                    href={item.href}
                                    className={clsx(
                                        'px-3 py-1.5 rounded-full transition-all duration-200 text-[18px] whitespace-nowrap',
                                        isActive
                                            ? (isDark ? 'bg-white text-black font-bold' : 'bg-black text-white font-bold')
                                            : (isDark ? 'text-white hover:bg-[#1F1F1F] font-semibold' : 'text-black hover:bg-gray-100 font-semibold')
                                    )}
                                >
                                    {item.label}
                                </Link>
                    ...
                    <a
                        href="mailto:osmankoycu@gmail.com"
                        className={clsx(
                            "shrink-0 ml-2 px-1 py-1 cursor-pointer transition-colors duration-300 hover:text-[#1F1F1F]",
                            isDark ? "text-white" : "text-black"
                        )}
                        aria-label="Send email"
                    >
